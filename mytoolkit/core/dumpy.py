...
# 	@staticmethod
# 	def encrypt(plaintext:bytes, publickey_int:int):
		
# 		ciphertext = b''

# 		for byte in plaintext:
# 			msg_byte = byte.to_bytes((byte.bit_length() + 7) // 8, 'big')
# 			h1 = hmac.new(b'secret-key',msg_byte, hashlib.sha1)        
# 			msg=int(byte)

# 			ciphertext_int = pow(msg, 2, publickey_int)
...
# 		MI1=pow(M1, p-2, p)
# 		MI2=pow(M2, q-2, q)
# 		P=((a*M1*MI1)+(b*M2*MI2))%M
# 		return int(P)

# 	@staticmethod
# 	def decyption(ciphertext_int, p, q):
# 		a1=pow(ciphertext_int, (p + 1) // 4, p)
# 		a2=p - a1
# 		b1= pow(ciphertext_int, (q + 1) // 4, q)
# 		b2=q - b1
# 		P1=rabin.__CRT(a1,b1,p,q)
# 		P2=rabin.__CRT(a1,b2,p,q)
# 		P3=rabin.__CRT(a2,b1,p,q)
# 		P4=rabin.__CRT(a2,b2,p,q)

# 		return [P1,P2,P3,P4]
		

# 	@staticmethod
# 	def Rabin(p, q, plaintext) -> bytes:
# 		dec_b = b''
# 		n=p*q
# 		for byte in plaintext:
# 			msg_byte=byte.to_bytes((byte.bit_length() + 7) // 8, 'big')
...
# 			for i in D:
# 				rcv_byte = i.to_bytes((i.bit_length() + 7) // 8, 'big')           
# 				h2 = hmac.new(b'secret-key', rcv_byte, hashlib.sha1)
# 				if(h2.digest()==h1.digest()):
# 					dec_b += rcv_byte
# 					break
